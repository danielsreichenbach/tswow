...
export namespace Config {
    ...
    export async function create() {
        ...
        ipaths.startJs.write(`require('child_process').execSync(\`${isWindows() ? '"bin/node/node.exe"' : 'node'} --enable-source-maps bin/scripts/runtime/runtime/TSWoW.js \${process.argv.slice(1).join(' ')}\`, { stdio: 'inherit' })`)
