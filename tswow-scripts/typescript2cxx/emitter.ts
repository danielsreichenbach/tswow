        ...
        if (writeAsLambdaCFunction) {
            if (isFunctionOrMethodDeclaration) {
                ...
                try { // doing try/catch here because "kind" seems to completely bug out when accessing parent elements
                    ...
                    let index = -1;
                    // Only inspect arguments when the parent really is a CallExpression — for arrow
                    // functions inside e.g. variable initializers it's not, and the unguarded cast
                    // used to throw and fall straight into the catch.
