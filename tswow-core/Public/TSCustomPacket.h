...
{
	// Held as a shared_ptr so copies of TSPacketWrite share ownership instead of
	// each calling Destroy() and double-freeing the chunks. The custom deleter
	// runs Destroy() (frees the chunk buffers) AND `delete` (frees the object
	// header itself, which the previous code leaked).
